...
export default class LinePage extends BasePage {
  ...
  generateLine (previousLine, spacing) {
    ...
    for (let i = 0; i < previousLine.length; i++) {
      ...
      let indices = previousIndices.filter((j) => distance(x, y, previousLine[j][0], previousLine[j][1]) <= spacing)

      // add node if not too close to a previous node _or_ it's the last point
      if (intersection(indices, nearLast).length < 5 || i === previousLine.length - 1) {
        nearLast = indices
    ...
    let knots = concat(
      [0, 0, 0, 0],
      range(2, rawLength - 2),
      [rawLength - 1, rawLength - 1, rawLength - 1, rawLength - 1]
    )
