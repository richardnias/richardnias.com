...
export default class MountainPage extends BasePage {
  ...
  init() {
    ...
    this.theta = 0;
    this.height =
      data[WORLD_WIDTH / 2 + (WORLD_DEPTH / 2) * WORLD_WIDTH] * 10 + 500;
    this.data = data;
  ...
  animate() {
    ...
    const xIndex =
      WORLD_WIDTH / 2 + Math.round((x / PLANE_WIDTH) * WORLD_WIDTH);
    const zIndex =
      WORLD_DEPTH / 2 + Math.round((z / PLANE_DEPTH) * WORLD_DEPTH);
    ...
    if (oldHeight < groundPosition + 300) {
      y = Math.max(
        weightedAvg(groundPosition + 300, oldHeight),
        groundPosition
      );
