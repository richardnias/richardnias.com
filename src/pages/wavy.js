...
export default class WavyPage extends WebcamPage {
  ...
  getTargetPoints() {
    return this.corners.concat(this.innerSourcePoints.map(fuzzMap));
  }
  ...
  animate() {
    ...
    for (let i = 0; i < source.triangles.length; i += 3) {
      ...
      mat3.set(this.sourceMatrix, s0x, s0y, 1, s1x, s1y, 1, s2x, s2y, 1);

      mat3.set(this.targetMatrix, t0x, t0y, 1, t1x, t1y, 1, t2x, t2y, 1);

      mat3.invert(this.invSourceMatrix, this.sourceMatrix);
      mat3.multiply(
        this.transformMatrix,
        this.targetMatrix,
        this.invSourceMatrix
      );
