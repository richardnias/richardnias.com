...
export default class SinePage extends BasePage {
  ...
  getYPosition (n, t) {
    return this.halfHeight * Math.sin(this.getXPosition(n) * t * SPEED_FACTOR) + this.halfHeight
  }
  ...
  animate () {
    ...
    let points = range(-this.numPoints / 2, this.numPoints / 2 + 1).map(n => ({
      x: this.getXPosition(n) + this.halfWidth,
      y: this.getYPosition(n, t),
      color: `hsl(${n / this.numPoints * 360 * 2}, 100%, ${n === 0 ? 100 : 30}%)`
    }))
