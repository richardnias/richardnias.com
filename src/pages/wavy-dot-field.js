...
export default class SunflowerPage extends BasePage {
  ...
  drawPoints (ctx, points, t) {
    ...
    points.forEach(function ({ x, y, thetaOffset }) {
      ctx.fillRect(x + Math.cos(thetaOffset + t) * WAVE_RADIUS - POINT_RADIUS, y + Math.sin(thetaOffset + t) * WAVE_RADIUS - POINT_RADIUS, POINT_RADIUS * 2, POINT_RADIUS * 2)
    })
  }
  ...
  onResize () {
    ...
    let numHorizontalPoints = Math.floor((window.innerWidth - MARGIN * 2) / WAVE_RADIUS * DENSITY)
    let numVerticalPoints = Math.floor((window.innerHeight - MARGIN * 2) / WAVE_RADIUS * DENSITY)
    ...
    for (let i = 0; i < numHorizontalPoints; i++) {
      for (let j = 0; j < numVerticalPoints; j++) {
        this.points.push({
          ...
          thetaOffset: (i / numHorizontalPoints * Math.cos(waveDirection) + j / numVerticalPoints * Math.sin(waveDirection)) * Math.PI * FREQUENCY
        })
      }
    }
  }
